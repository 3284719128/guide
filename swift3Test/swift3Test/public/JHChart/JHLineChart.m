...
//        self.backgroundColor = [UIColor colorWithRed:0.2 green:0.7 blue:0.2 alpha:0.3];
        ...
        _lineWidth = 0.5;
        self.contentInsets = UIEdgeInsetsMake(10, 20, 25, 10);
...
- (void)updateYScale{
        ...
        default:{
            ...
            if (_valueArr.count) {
                ...
                min = [_valueArr[0][0] integerValue];
                ...
                NSMutableArray *arr = [NSMutableArray array];
                    ...
                    for (NSInteger i = min/200; i<=max/200; i++) {
                        [arr addObject:[NSString stringWithFormat:@"%ld",i*200]];
        ...
        case JHLineChartQuadrantTypeFirstQuardrant:{
            ...
            if (_xLineDataArr.count>0) {
                ...
                for (NSInteger i = 0; i<_xLineDataArr.count;i++ ) {
                    ...
                    [self drawText:[NSString stringWithFormat:@"%@",_xLineDataArr[i]] andContext:context atPoint:P_M(p.x-len/2, p.y+7) WithColor:_xAndYNumberColor andFontSize:12.0];
                }
...
//                CGFloat yPace = (_yLength - kXandYSpaceForSuperView - heightZero)/(_yLineDataArr.count - 1);
//                for (NSInteger i = 0; i<_yLineDataArr.count; i++) {
//                    CGPoint p = P_M(self.chartOrigin.x, self.chartOrigin.y - i*yPace - heightZero);
...
//            _perValue = _perYlen/[[_yLineDataArr firstObject] floatValue];
            _perValue = (_yLength-kXandYSpaceForSuperView-heightZero) / (max - min);
            
            
            
            for (NSArray *valueArr1 in _valueArr) {
                NSMutableArray *dataMArr = [NSMutableArray array];
                for (NSInteger i = 0; i<valueArr1.count; i++) {
                    
                    CGPoint p = P_M(i*_perXLen+self.chartOrigin.x,self.contentInsets.top + _yLength - ([valueArr1[i] floatValue] - min )*_perValue - heightZero);
...
//                        [self drawText:[NSString stringWithFormat:@"(%@,%@)",_xLineDataArr[i],_valueArr[m][i]] andContext:context atPoint:p WithColor:pointNumberColor andFontSize:7.0];
                        [self drawText:[NSString stringWithFormat:@"%@",_valueArr[m][i]] andContext:context atPoint:P_M(p.x - 14, p.y - 14) WithColor:pointNumberColor andFontSize:12.0];
                    }
                        break;
                    case JHLineChartQuadrantTypeFirstAndSecondQuardrant:
                    {
